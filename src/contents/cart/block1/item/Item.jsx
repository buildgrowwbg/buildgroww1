...
export default function Item({ index, product }) {
  ...
  const [value, setValue] = useState(product.products[0].qty);
  const [click, setClick] = useState(0);
  const [open, setOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const [openDialog, setOpenDialog] = useState(false);
  const [addOn, setAddOn] = useState([...product.products[0].addonId]);
  ...
  useEffect(() => {
    async function fetchdata() {
      if (click) {
        const add = [];
        cart.products?.data?.data[index]?.products[0].addonId.map((item)=>{
            add.push(item.id)
        ...
        const updateList = {
          products: [
            {
              productId:
                cart.products?.data?.data[index]?.products[0].productId.id,
        ...
        const resUpdate = await updateCartProduct(product.id, updateList);
  ...
  const handleDelete = async () => {
    ...
    const resUpdate = await deleteCart(product.id);
    setLoading(false);
    if (resUpdate?.data?.status === "SUCCESS") {
  ...
  const handleCancel = ()=>{
      setOpenDialog(false);
      setAddOn([...product.products[0].addonId])
  }
  const handleSave = ()=>{
  ...
  return (
    ...
    >
      ...
      >
        <Image
          src={`${
            ...
            product.products[0].productId.productImages &&
            product.products[0].productId.productImages[0].path
          }`}
          alt={`${product?.products[0]?.productId?.title?.shortTitle}`}
        />
        <Box
          sx={{
          ...
          >
            ...
            >
              <Paragraph sx={{ fontSize: "18px",fontWeight:600 }}>
                {product &&
                  product.products[0].productId &&
                  product.products[0].productId.title.shortTitle}
              </Paragraph>
              <Paragraph sx={{ fontSize: "18px" }}>
                <span style={{ fontWeight: 500 }}>Package : </span>
                {product &&
                  product.products[0].packageId &&
                  product.products[0].packageId.name}
              </Paragraph>
              ...
              <Box>
                ...
                >
                  ...
                  >
                    ...
                    {product &&
                      product.products[0].packageId &&
                      product.products[0].packageId.price * product.products[0].qty}
                    /-{" "}
                  </Paragraph>
                  {/* <Box
                    sx={{
                  ...
                  >
                    <FaShippingFast />
                    <Paragraph variant="span">Delivery after</Paragraph>
      ...
      {/* <Divider/> */}
      {/* {product?.products[0]?.addonId?.length !==0 ?<Box sx={{height:50,display:'flex',justifyContent:'center',alignItems:'center',borderTop:`1px solid ${theme.colors.alpha.black[50]}`}}> <Paragraph sx={{fontSize:{sm:'22px',xs:'16px'},fontWeight:500}}>Addons</Paragraph></Box> : null} */}
      {product?.products[0]?.addonId?.length !==0 ? product.products[0].addonId.map((item,index) => {
          return (
            // <Box>
